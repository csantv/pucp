...
asignacion:   ID {$$=localizaSimbolo(lexema,ID,1);} ':' expr {generarCodigo(ASIGNACIONS,$2,$4,'-');} '#';
...
showDate:
    IMPFECHA ID
    {$$=localizaSimbolo(lexema,ID,1);}
    {generarCodigo(SHOWD,$3,'-','-');} '#'
;

showAge:
    MOSTEDAD paramAge ',' paramAge '#'
    {generarCodigo(SHOWA,$2,$4,'-');}
;
...
toDate:
    AFECHA ID {$$=localizaSimbolo(lexema,ID,1);} ':'
    CADENA {$$=localizaSimbolo(lexema,CADENA,1);} '#'
    {generarCodigo(TODATE,$3,$6,'-');}
;
