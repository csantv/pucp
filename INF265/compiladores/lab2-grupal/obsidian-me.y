...
declaracion:
    DECLARAR listaDecl ':' TIPO '#'
;

listaDecl:
    ID                             {$$=localizaSimbolo(lexema,ID);}
|   listaDecl ',' ID               {$$=localizaSimbolo(lexema,ID);}
;
...
|   CADENA ',' listaCads    {$$=localizaSimbolo(lexema,CADENA);} 
;
...
switch:
    SWITCH ID  ':' listaCasosSwitch '#' {$$=localizaSimbolo(lexema,ID);}
;
...
casoSwitch:
    CASOSWITCH NUMBER ':' listainst '#' {$$=localizaSimbolo(lexema,NUMBER);}
...
varRecShow:
    paramShow ',' varRecShow
|   paramShow ;

showLine:
    IMPLINEA CARACTER {$$=localizaSimbolo(lexema,CARACTER);} '#'
;
...
showDate:
    IMPFECHA FORMATO_FECHA '(' ID {$$=localizaSimbolo(lexema,ID);}  ',' CADENA {$$=localizaSimbolo(lexema,CADENA);} ')' ;
